...
5. **Per-recipe breakdown** — click the ▸ next to any ingredient to expand
   a table of every dish/cook that used it, with the quantity exactly as it
   was written in the source spreadsheet.
6. **Store sorting** (`suppliers.js`) — every ingredient is assigned to a
   store, learned from the spreadsheet's "Supplier" column:
   - If an ingredient was explicitly linked to a supplier in the sheet
     (majority vote if it saw more than one), that's used directly.
   - That learning is saved in the browser's `localStorage`, so an
     ingredient with no supplier listed in *this* file can still be
     auto-assigned based on what it was linked to in a *previous* upload.
   - Anything never seen with a supplier, ever, lands in an "Unassigned"
     section.
   - Picking a store from the dropdown on any item corrects it immediately
     and is weighted higher than spreadsheet history, so a manual
     correction sticks even against a lot of past mentions.
   - This learning is local to the browser/device — it won't follow you to
     a different computer, and clearing site data / browser storage will
     reset it.
