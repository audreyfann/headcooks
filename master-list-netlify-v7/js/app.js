  ...
  for (const storeName of storeNames) {
    ...
    items.forEach((r, idx) => {
      ...
      const linesHtml = r.lines
        .map((line) => {
          const estLbHtml =
            line.estimatedLb != null
              ? ` <span class="est-lb" title="Estimated using an average ${escapeHtml(r.ingredientNormalized)} weight -- not a substitute for the actual count above.">(~${line.estimatedLb} lb)</span>`
              : "";
          return `<div class="qty-line"><span class="num">${line.displayQty}</span> ${escapeHtml(line.displayUnit)}${estLbHtml}${line.isEstimated ? '<span class="est-flag" title="Estimated: weight and volume mentions were combined using a generic density, since this ingredient has no known density on file.">~est</span>' : ""}</div>`;
        })
        .join("");
      ...
      const hasNotes = r.sourceRows.some((s) => s.notes);
      const noteIndicator = hasNotes
        ? '<span class="note-flag" title="At least one recipe has a note for this ingredient -- expand to view">note</span>'
        : "";

      mainRow.innerHTML = `
        <td>
          <button class="expand-toggle" aria-expanded="false" aria-label="Show per-recipe breakdown">▸</button>
          ${escapeHtml(r.ingredientNormalized)}${noteIndicator}
        </td>
      ...
      const detailItems = r.sourceRows
        .map(
          (s) =>
            `<tr><td>${escapeHtml(s.dish || "(no dish listed)")}</td><td>${escapeHtml(s.cook)}</td><td class="num">${escapeHtml(s.quantity || "\u2014")}</td><td>${escapeHtml(s.supplier || "\u2014")}</td><td class="notes-cell">${escapeHtml(s.notes || "\u2014")}</td></tr>`
        )
        .join("");
      detailRow.innerHTML = `
        <td colspan="5">
          <table class="detail-table">
            <thead><tr><th>Dish</th><th>Cook</th><th class="num">Quantity (as written)</th><th>Supplier (as written)</th><th>Notes</th></tr></thead>
...
downloadCombinedBtn.addEventListener("click", () => {
  ...
  for (const r of latestCombined) {
    ...
    for (const line of r.lines) {
      flatRows.push({
        ...
        estimated_lb: line.estimatedLb != null ? line.estimatedLb : "",
  ...
  const csv = toCsv(flatRows, [
    ...
    { key: "unit", label: "unit" },
    { key: "estimated_lb", label: "estimated_lb" },
