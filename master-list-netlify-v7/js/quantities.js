...
// Returns [number, unitCode, remainderText]. unitCode is a UNIT_MAP value, or
// "count" if a number was found with no recognized unit word (in which case
// remainderText is whatever followed the number, e.g. "onions" in "20
// onions", or "large onions", or "bags" in "2 bags" -- useful downstream for
// telling a genuine object count apart from a container/packaging count).
// Returns [null, null, null] if nothing parseable was found (e.g. "as needed").
export function parseQuantity(raw) {
  // A spreadsheet cell that's a genuine number (not text) -- e.g. someone
  // typed 20 directly rather than "20" -- has no unit word to look for at
  // all, so it's unambiguously just a count of however many there are.
  if (typeof raw === "number" && Number.isFinite(raw)) {
    return [raw, "count", ""];
  }

  if (typeof raw !== "string" || !raw.trim()) return [null, null, null];
  ...
  if (rangeMatch) {
    ...
    if (!numMatch) return [null, null, null];
...
export function addParsedQuantities(rows) {
  return rows.map((row) => {
    const [qtyNum, qtyUnit, qtyRemainderText] = parseQuantity(row.quantity);
    const [qtyBase, unitCategory] = convertRow(row.ingredientNormalized, qtyNum, qtyUnit);
    return { ...row, qtyNum, qtyUnit, qtyRemainderText, qtyBase, unitCategory };
  });
}
